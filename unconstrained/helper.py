...
def German(f):
    ...
    test = pd.read_csv("data/german_test.csv")
    test['pred'] = y_pred
    test.to_csv("results_unconstrained/german_test_repaired.csv", index=False)
    np.savetxt("results_unconstrained/german_test_repaired_cd.csv", y_cd, delimiter=",")
