...
def Adult(f):
    ...
    df = pd.read_csv(f)
    df = df[keep]
    test = pd.read_csv("results_Calmon/adult_test_notrepaired.csv")
    test = test[keep]
    ...
    test = pd.read_csv("results_Calmon/adult_test_notrepaired.csv")
    test['pred'] = y_pred
    test.to_csv("results_Calmon/adult_test_repaired.csv", index=False)
    np.savetxt("results_Calmon/adult_test_repaired_cd.csv", y_cd, delimiter=",")
 
    
    
def Compas(f, t="results_Calmon/compas_test_notrepaired.csv", f1='', f2=''):
    X_int = []
    X_cat = ['Prior', 'Age', 'Sex']
    ...
    df = pd.read_csv(f)
    df = df[keep]
    test = pd.read_csv("results_Calmon/compas_test_notrepaired.csv")
    test = test[keep]
    df = pd.concat([df, test])
    ...
    np.savetxt(f1+"results_Calmon/compas_test_repaired"+f2+"_cd.csv", y_cd, delimiter=",")
...
def German(f):
    ...
    df = pd.read_csv(f)
    df = df[keep]
    test = pd.read_csv("results_Calmon/german_test_repaired.csv", header=0, delimiter=',',)
    test = test[keep]
    df = pd.concat([df, test])
    ...
    test = pd.read_csv("results_Calmon/german_test_notrepaired.csv")
    print(test)
    test['pred'] = y_pred
    test.to_csv("results_Calmon/german_test_repaired.csv", index=False)
    np.savetxt("results_Calmon/german_test_repaired_cd.csv", y_cd, delimiter=",")
...
#compute_metrics('german', "results_Calmon/german_train_repaired.csv")