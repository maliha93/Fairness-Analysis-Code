...
def Adult(f):
    ...
    df = pd.read_csv(f)
    df = df[keep]
    _, test = pd.read_csv("../data/Adult_bin.csv").train_test_split(test_size=0.3, shuffle=False, random_state=42)
    test = test[keep]
    ...
    test['pred'] = y_pred
    test.to_csv("../results_Feldman/adult_test_repaired.csv", index=False)
    np.savetxt("../results_Zhang/adult_test_repaired_cd.csv", y_cd, delimiter=",")
    
    
def Compas(f, t="../data/Compas_bin.csv", f1='', f2=''):
    X_int = []
    X_cat = ['Prior', 'Age', 'Sex']
    ...
    _, test = train_test_split(pd.read_csv(t), test_size=0.3, shuffle=False, random_state=42)
    test['pred'] = y_pred
    test.to_csv("../"+f1+"results_Zhang/compas_test_repaired"+f2+".csv", index=False)
...
def German(f):
    ...
    df = pd.read_csv(f)
    df = df[keep]
    _, test = train_test_split(pd.read_csv("../data/German_bin.csv"), test_size=0.3, shuffle=False, random_state=42)
    test = test[keep]
    df = pd.concat([df, test])
...
#compute_metrics('german', "results_Zhang/german_train_repaired.csv")