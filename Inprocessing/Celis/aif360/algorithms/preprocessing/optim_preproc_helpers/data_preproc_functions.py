...
def load_preproc_data_credit(protected_attributes=None, sub_samp=False, balance=False, optim=False, columns=None, fname=None):
    def custom_preprocessing(df):
        
        
        # Recode sex and race
        df['SEX'] = df['SEX'].replace({2 : 0.0, 1 : 1.0})
    ...
    if optim:
            XD_features = ['EDUCATION', 'SEX', 'MARRIAGE', 'LIMIT_BAL', 'AGE', 'PAY_0',
                   'PAY_2']
    if columns:
        XD_features = columns
        categorical_features = list(set(XD_features)-set(D_features))
        
    else:
        XD_features = ['LIMIT_BAL', 'SEX', 'EDUCATION', 'MARRIAGE',  'AGE', 'PAY_0', 
                   'PAY_AMT1']
        categorical_features = ['EDUCATION', 'MARRIAGE', 'LIMIT_BAL', 'AGE', 'PAY_0',
                            'PAY_AMT1']
