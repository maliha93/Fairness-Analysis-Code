...
def get_clf_stats(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs):
    ...
    train_score, test_score, correct_answers_train, correct_answers_test, y_pred = ut.check_accuracy(w, x_train, y_train, x_test, y_test, x_control_test, None, None)
    ...
    return train_score, test_score, cov_all_train, cov_all_test, s_attr_to_fp_fn_train, s_attr_to_fp_fn_test, y_pred
...
def get_fpr_fnr_sensitive_features(y_true, y_pred, x_control, sensitive_attrs, verbose = False):
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            acc = float(sum_array(y_true_local==y_pred_local)) / len(y_true_local)

            fp = sum_array(np.logical_and(y_true_local == -1.0, y_pred_local == +1.0)) # something which is -ve but is misclassified as +ve
            fn = sum_array(np.logical_and(y_true_local == +1.0, y_pred_local == -1.0)) # something which is +ve but is misclassified as -ve
            tp = sum_array(np.logical_and(y_true_local == +1.0, y_pred_local == +1.0)) # something which is +ve AND is correctly classified as +ve
            tn = sum_array(np.logical_and(y_true_local == -1.0, y_pred_local == -1.0)) # something which is -ve AND is correctly classified as -ve

            all_neg = sum_array(y_true_local == -1.0)
            all_pos = sum_array(y_true_local == +1.0)
...
def get_sensitive_attr_constraint_fpr_fnr_cov(model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr, verbose=False):
    ...
    for v in set(x_control_arr):
        s_val_to_total[0][v] = sum_array(x_control_arr == v)
        s_val_to_total[1][v] = sum_array(np.logical_and(x_control_arr == v, y_arr_true == -1))
        s_val_to_total[2][v] = sum_array(np.logical_and(x_control_arr == v, y_arr_true == +1))
    ...
    for v in set(x_control_arr):
        ...
        cons_sum_dict[0][v] = sum_array( np.minimum(0, dist_bound_prod) ) * (s_val_to_avg[0][v] / len(x_arr))
        cons_sum_dict[1][v] = sum_array( np.minimum(0, ( (1 - y_arr_true[idx]) / 2 ) * dist_bound_prod) ) * (s_val_to_avg[1][v] / sum(y_arr_true == -1))
        cons_sum_dict[2][v] = sum_array( np.minimum(0, ( (1 + y_arr_true[idx]) / 2 ) * dist_bound_prod) ) * (s_val_to_avg[2][v] / sum(y_arr_true == +1))
