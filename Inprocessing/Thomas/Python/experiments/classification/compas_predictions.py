...
if __name__ == '__main__':
	...
	dataset = compas.load(use_pct=args.data_pct, r_train=args.r_train_v_test, r_candidate=args.r_cand_v_safe, include_intercept=True)
	...
	e = time.time()
	print("Time:", (e-s))
	df = pd.read_csv("datasets/propublica/Compas.csv")
	df = df.iloc[df.shape[0]-X.shape[0]:, :]
	df['pred_h'] = Yp_h
	df['pred_t'] = Yp_t
	df.to_csv("results_Thomas/compas_test_"+args.definition.lower()+"_repaired.csv", index=False)   
	...
	#fname = 'results/compas_predictions/%s.npz' % args.definition.lower()
	#os.makedirs('results/compas_predictions/', exist_ok=True)
